...
def lr_auc(var_size=10):
    ...
    data.to_csv("./result/auc_curv.csv",index=False)

    
def rf_res():
    df = py.read_r("./pos_data/data_imp.rds")[None]
    df["sex"]=pd.get_dummies(df["gender"]).iloc[:,0]
    del df["Sample"]
    del df["gender"]
    del df["ST1: batch"]
